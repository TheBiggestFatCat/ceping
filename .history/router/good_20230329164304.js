...
router.post("/addgood",(req,res)=>{//添加一个商品 goodName,goodProducterId,goodPicture,goodDate,goodPrice,goodtext
    ...
    db.query(sqlstr1,info.goodProducterId,(err,result)=>{
        ...
        if(result.length!=1||result[0].role!=2){return res.send({status:1,message:"没这公司或有多个同id公司"})}
        if(result[0].role==2){
            const sqlstr2="insert into good set ? "
            db.query(sqlstr2,{goodName:info.goodName,goodProducterId:info.goodProducterId,goodPicture:info.goodPicture,
                            goodDate:info.goodDate,goodPrice:info.goodPrice,goodText:info.goodText},
                (err,result)=>{
                    if(err){return res.send({status:1,message:err.message})}
...
router.post("/getrandgood",(req,res)=>{//查询所有物品
    const info=req.query

    const sqlstr="SELECT * FROM good WHERE goodName LIKE ‘%" + info.search + "%’"
    db.query(sqlstr,(err,result)=>{
        if(err){return res.send({status:1,message:err.message})}
        res.send({
            status:0,
