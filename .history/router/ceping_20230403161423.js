            ...
            db.query(sqlstr2, info.cepingGoodId, (err, result) => {
                ...
                db.query(sqlstr3, [info.cepingerId, info.cepingGoodId], (err, result) => {
                    ...
                    if (result.pop().shenqingStatus == 2) {
                        ...
                        db.query(sqlstr, {
                                cepingText: info.cepingText,
                                cepingerId: info.cepingerId,
                                cepingGoodId: info.cepingGoodId,
                                cepingDate: info.cepingDate,
                                cepingPicture: info.cepingPicture
                            },
                            (err, result) => {
                                if (err) {
                                    return res.send({
...
router.get("/getcepingbygoodid", (req, res) => { //商品id查测评
    ...
    console.log(info);
    const sqlstr = "select * from ceping.ceping where cepingGoodId=?"
