...
router.get("/getshenqingbyid",(req,res)=>{//根据企业id查询所有申请
    const sqlstr="select * from good where goodProducterId=" + req.query.id
    let resList = []
    db.query(sqlstr,(err,result)=>{
        if(err){return res.send({status:1,message:err.message})}
        for (let i of result) {
            const sqlstr2 = "select * from shenqing where shenqingGoodId=" + i.goodId
            db.query(sqlstr2, (err, result2) => {
                if (err) {
                    return res.send({
                        status: 1,
                        message: err.message
